...
editable in the "Paths" expander at the top. DATE_EVENTS_RAW_DIR belongs to the date-events
page and is set there.
"""
...
from line_highlight import document_only_page
from loader import (
    L1_DIR_VARS,
    claim_rows,
    list_entities,
...
from state import is_dark, paths_expander, sticky
from styles import claim_card, crumb, field_table, reason_items, section
...
def render_claims(record: dict[str, Any], configured: dict | None,
    ...
    for role, field in configured["dates"].items():
        ...
        if row:
            ...
            # every document carrying the matched text, not just the first -- the docId
            # is how you find it in the Date event evidence tab, so it stays in full
            docs = row_docs(row)
            for position, doc in enumerate(docs, start=1):
                suffix = f" {position}" if len(docs) > 1 else ""
                fields += [
                    (f"Doc ID{suffix}", doc.get("docId") or "", "doc"),
                    (f"Doc{suffix}",
                     f"{doc.get('doctype') or '?'}  ·  {doc.get('docDate') or '—'}", "doc"),
                ]
...
def row_docs(row: dict[str, Any]) -> list[dict[str, Any]]:
    """
    Every document supporting a verdict row.

    The judge now writes `docs` -- a list, because one sentence on one date routinely
    comes from several documents. Results written before that change carry only the
    singular fields, so fall back to those rather than showing nothing.
    """
    docs = row.get("docs")
    if isinstance(docs, list) and docs:
        return [d for d in docs if isinstance(d, dict)]
    if row.get("docId"):
        return [{"docId": row["docId"], "docDate": row.get("docDate"),
                 "doctype": row.get("doctype")}]
    return []


def collect_docs(
    record: dict[str, Any], judge_rows: list[dict[str, Any]], patient_id: str
    ...
    for row in judge_rows or []:
        ...
        for doc in row_docs(row):
            doc_id = doc.get("docId")
            if not doc_id:
                continue
            # the judge never sees citations; recover them from the merged index so this
            # side highlights exact LINES instead of searching for the text
            citations = merged_citations(patient_id, row.get("matched_date"), text, doc_id)
            judge.setdefault(doc_id, []).append(({"text": text, "citations": citations}, note))

    return l1, judge


def render_doc_panel(title: str, docs: dict[str, list[tuple[dict, str]]],
                     patient_id: str, height: int = 420, by_lines: bool = False) -> None:
    ...
    # two citation shapes, two highlighters: the date-event side carries LINE ranges
    # and highlights exactly; L1's own citations are text/offsets and need searching
    if by_lines:
        page, found = document_only_page(doc_text, citations, notes, dark=is_dark())
        missing = [n for n in range(1, len(citations) + 1)][found:]
        numbers = list(range(1, found + 1))
    else:
        page, located = document_page(doc_text, citations, notes, dark=is_dark())
...
paths_expander(L1_DIR_VARS, optional=("PATIENT_DATA_DIR",))
...
entity_key = entity_labels[sticky(top_entity, "Entity", list(entity_labels), key="entity")]
...
picked = sticky(top_record, f"Record — {len(records)} in list", list(record_labels),
                key="record")
...
l1_docs, judge_docs = collect_docs(record, judge_rows, patient_id)
...
with evidence_col:
    ...
    with event_tab:
        render_doc_panel("Date event evidence documents", judge_docs, patient_id,
                         height=520, by_lines=True)
    with reason_tab:
